...
| Screening | Possibility decisions per concept | Survivors + candidate cutoff rules |
| Confirm Rules | Respondent confirms/rejects detected rules | Confirmed must-have / unacceptable rules |
...
Each screening response becomes a binary observation in the design matrix with a `none_threshold` column set to 1.

## Must-have and unacceptable detection

After enough screens, the engine scans the response history for systematic patterns:

- **Must-have**: a level is always chosen when shown.
...
If a confirmed rule invalidates planned concepts, the engine enters `REGENERATE`, filters the concept pool, and fills it back to size `T` with replacement concepts that satisfy every confirmed rule. This phase is transparent to the respondent.
...
Surviving concepts (those marked possible) compete in triples. The number of rounds is `floor(t/2)` where `t` is the number of survivors. The respondent picks the preferred concept in each triple. Ties are resolved by a deterministic coin flip seeded per matchup.
