 ...
 */
export function renderACBCSurvey(survey, engine, options = {}) {
    const resolvedOptions = {
        Survey: options.Survey ?? (typeof window !== "undefined" ? window.Survey : undefined),
 ...
 */
export function onStateChange(survey, callback) {
    // Store callback on the survey's custom properties
    survey.__acbc_onStateChange = callback;
}
// ---------------------------------------------------------------------------
// Page building
// ---------------------------------------------------------------------------
function buildSurveyPages(adapter) {
...
function buildBYOPage(adapter) {
    ...
    page.visibleIf = "{__acbc_phase} = 'BYO'";
}
// ---------------------------------------------------------------------------
// Screening Page — concept cards with Possible / Not Possible
// ---------------------------------------------------------------------------
function buildScreeningPage(adapter) {
    const { survey, engine, options } = adapter;
    const config = engine.getConfig();
    const { cssPrefix } = options;
    const page = survey.addPage({
    ...
    page.visibleIf = "{__acbc_phase} = 'SCREENING'";
}
// ---------------------------------------------------------------------------
// Confirmation Page — must-have / unacceptable rule confirmation
// ---------------------------------------------------------------------------
function buildConfirmationPage(adapter, kind) {
    ...
    page.visibleIf = `{__acbc_phase} = '${phaseValue}'`;
}
// ---------------------------------------------------------------------------
// Tournament Page — triple elimination with grayed shared attributes
// ---------------------------------------------------------------------------
function buildTournamentPage(adapter) {
    const { survey, options } = adapter;
    const { cssPrefix } = options;
    const page = survey.addPage({
    ...
    page.visibleIf = "{__acbc_phase} = 'TOURNAMENT'";
}
// ---------------------------------------------------------------------------
// Calibration Page — Likert scale purchase intent
// ---------------------------------------------------------------------------
function buildCalibrationPage(adapter) {
    const { survey, engine, options } = adapter;
    ...
    page.visibleIf = "{__acbc_phase} = 'CALIBRATION'";
}
// ---------------------------------------------------------------------------
// Completion Page
// ---------------------------------------------------------------------------
function buildCompletionPage(adapter) {
    const { survey, options } = adapter;
    const { cssPrefix } = options;
    const page = survey.addPage({
        name: "page-done",
    ...
    page.visibleIf = "{__acbc_phase} = 'DONE'";
}
// ---------------------------------------------------------------------------
// Phase navigation and visibility
// ---------------------------------------------------------------------------
function updatePageVisibility(adapter, state) {
...
function navigateToCurrentPhase(adapter, state) {
    const { survey } = adapter;
...
function mapPhaseToMarker(phase) {
    return phase;
}
function mapPhaseToPageName(phase) {
    switch (phase) {
        case "BYO":
            return "page-byo";
        case "SCREENING":
...
// ---------------------------------------------------------------------------
function setupStateChangeListener(adapter) {
...
function handlePageChange(adapter, options) {
    const { survey, engine } = adapter;
    const state = engine.getState();
    ...
    if ((newPage.name === "page-confirm-must-have" ||
    ...
    if ((newPage.name === "page-screening" || newPage.name === "page-tournament") &&
    ...
    if (newPage.name === "page-calibration" || newPage.name === "page-done" &&
...
function handleValueChange(adapter, options) {
    const { survey, engine } = adapter;
    const state = engine.getState();
...
// ---------------------------------------------------------------------------
function submitBYOEvent(adapter) {
    const { survey, engine } = adapter;
    const config = engine.getConfig();
    const byoAttributes = config.study.attributes.filter((a) => a.in_byo);
    const answers = {};
...
function submitScreeningEvent(adapter) {
    const { survey, engine } = adapter;
    const state = engine.getState();
...
function submitRuleDecisionEvent(adapter) {
    const { survey, engine } = adapter;
    const state = engine.getState();
    const decisionKey = state.phase === "CONFIRM_MUST_HAVE"
...
function submitTournamentEvent(adapter) {
    const { survey, engine } = adapter;
    const state = engine.getState();
    ...
    if (!currentTask)
        return;
    const selectedConceptId = survey.getValue("tournament-selection");
    const event = {
...
function submitCalibrationEvent(adapter) {
    const { survey, engine } = adapter;
    const state = engine.getState();
    const purchaseIntent = survey.getValue("calibration-purchase-intent");
    if (purchaseIntent === undefined || purchaseIntent === null)
        return;
    const winnerConcept = getTournamentWinnerConcept(state);
    if (!winnerConcept)
        return;
...
// ---------------------------------------------------------------------------
function afterStateChange(adapter, newState) {
    const { survey, engine } = adapter;
    // Update the phase marker
    survey.setValue("__acbc_phase", mapPhaseToMarker(newState.phase));
    // Rebuild pages if phase changed
    const oldPhase = survey.__acbc_lastPhase;
    if (oldPhase !== newState.phase) {
        buildSurveyPages(adapter);
    }
    survey.__acbc_lastPhase = newState.phase;
    // Re-render dynamic content
    renderDynamicContent(adapter);
    // Notify external listeners
    const callback = survey.__acbc_onStateChange;
    if (callback) {
        callback(newState);
    }
}
...
// ---------------------------------------------------------------------------
function renderDynamicContent(adapter) {
...
function renderPhaseContent(adapter, state) {
    const { engine, options } = adapter;
    const config = engine.getConfig();
    const { cssPrefix } = options;
    switch (state.phase) {
...
// ---------------------------------------------------------------------------
function renderScreeningConcepts(adapter, state, config) {
...
function renderConceptCard(concept, attributes, state, cssPrefix) {
...
// ---------------------------------------------------------------------------
function renderConfirmationMessage(adapter, state) {
    const { engine, options } = adapter;
    const config = engine.getConfig();
    const { cssPrefix } = options;
    ...
    if (!container)
        return;
    const message = rule.kind === "mustHave"
...
// ---------------------------------------------------------------------------
function renderTournamentTriple(adapter, state, config) {
    const { options } = adapter;
    const { cssPrefix } = options;
    ...
    const headerCells = concepts
        .map((_, i) => `<th class="${cssPrefix}-tournament-header">Option ${i + 1}</th>`)
        .join("");
    const attributeRows = attributes
        .map((attr) => {
...
// ---------------------------------------------------------------------------
function renderCalibrationConcept(adapter, state, config) {
    const { options } = adapter;
    const { cssPrefix } = options;
...
// Utility helpers
// ---------------------------------------------------------------------------
function getCurrentScreeningBatch(state) {
...
function getTournamentWinnerConcept(state) {
    if (state.tournamentRounds.length === 0)
        return null;
    const lastRound = state.tournamentRounds[state.tournamentRounds.length - 1];
    if (lastRound.tasks.length === 0)
        return null;
    const lastTask = lastRound.tasks[lastRound.tasks.length - 1];
    if (!lastTask.winnerConceptId)
        return null;
 ...
 */
export function injectACBCStyles(cssPrefix = DEFAULT_CSS_PREFIX) {
    if (typeof document === "undefined")
        return;
    if (document.getElementById(`${cssPrefix}-styles`))
        return;
...
//# sourceMappingURL=surveyjs-adapter.js.map